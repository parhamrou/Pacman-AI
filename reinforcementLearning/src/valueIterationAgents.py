...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
            ...
            # We select the Q with the max value
            max_value = -100000
            policy = self.mdp.getPossibleActions(state)[0]
            for action in self.mdp.getPossibleActions(state):
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for iteration in range(iteration_num):
            state_idx = iteration % len(states)
    ...
    def runValueIteration(self):
            ...
            if not self.mdp.isTerminal(state):
                ...
                for action in self.mdp.getPossibleActions(state):
                    ...
                    if Q_value > max_Q_value:
                        max_Q_value = Q_value
                diff = abs(max_Q_value - current_value)
        ...
        for iteration in range(self.iterations):
            ...
            for previous in predecessors[state]:
                Q_values = list()
                max_Q_value = -100000
                current_value = self.values[previous]
                ...
                if abs(current_value - max_Q_value) > self.theta:
                    queue.update(previous, -abs(current_value - max_Q_value))
